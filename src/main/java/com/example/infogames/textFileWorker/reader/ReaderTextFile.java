...
import com.example.infogames.textFileWorker.reader.SpaceWorker.SpaceWorker;
...
public class ReaderTextFile {
    ...
    private ParagraphSettings readParagraph(XWPFParagraph paragraph){
        ...
        paragraphSettings.setAlignment(paragraph.getAlignment().toString());
    ...
    private RunSettings readSettings(XWPFRun run){
        ...
        settings.setSizeFont(run.getFontSizeAsDouble() == null ? 12.0 : run.getFontSizeAsDouble());
        settings.setBold(run.isBold());
        settings.setItalic(run.isItalic());
        settings.setColorText(run.getColor() == null ? "black" : run.getColor());
        settings.setUnderline(run.getUnderline().toString());
        settings.setHighlighting(run.getTextHighlightColor().toString());
        settings.setStrikethrough(run.isStrikeThrough());
    ...
    private boolean areSettingsEqual(RunSettings s1, RunSettings s2) {
        ...
        return s1.getSizeFont() == s2.getSizeFont() &&
                ...
                Objects.equals(s1.getHighlighting(), s2.getHighlighting()) &&
